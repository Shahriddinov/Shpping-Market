...
const Passports = (key, value) => {
  ...
  async function addUser() {
    axios
      .post(`${baseApi}/pasport`, state.user, {
        headers: {
  ...
  useEffect(() => {
    axios
      .get(`${baseApi}/time_managment`, {
        headers: {
          "Accept-Language": localStorage.getItem("lng") || "uz",
        },
      })
      .then((res) => {
        setTime(res.data.timeManagment[0])
        console.log(res.data.timeManagment[0]);
      });
  }, []);
  ...
  function errorPasspotNum(e) {
    ...
    } else setEPassportNum(initialError);
  }
      ...
      <div className="right">
        <div className="container">
          <div className="col-md-10 offset-1">
            ...
            <span className="text-danger p-2">{t("register")}  {time?.day_from ?? ""} {t("times")} {time?.time_form ?? ""}  {t("to")}  {time?.day_to ?? ""}, {time?.time_to ?? ""} {t("at")} </span>
            ...
            <Button onClick={addUser} variant="outlined" className="sends">
              {" "}
              {t("countinue")}
            </Button>
          </div>
