...
const Direction = (props) => {
    ...
    function handleChange() {
        axios.get('https://sport.napaautomotive.uz/api/direction ', {
            headers: {
    ...
    async function handleChangeTwo(e) {
        ...
        axios.get(`https://sport.napaautomotive.uz/api/directionCategory`, {
            headers: {
                "Accept-Language": localStorage.getItem("lng",) || "uz"
            }
        }).then((response) => {
                            ...
                            >
                                {direction?.map((item, index) => {
                                    ...
                                    return <MenuItem key={index}
                                                     value={item.id}>{item.title_ru ?? item.title_uz ?? item.title_en}</MenuItem>
                        ...
                        <div className="NextPrev">
                            <Stack spacing={2} direction="row">
                                <Button className="button" variant="contained"
                                        href={`/profileOver/${localStorage.getItem("userId")}`}> <span
                                    className="icones"><CancelOutlinedIcon fontSize="small"/></span> Назад</Button>
                            </Stack>
                            <Stack spacing={2} direction="row">
                                <Button className="button" onClick={handleChangeTwo}
                                        href={`/allInfo/${localStorage.getItem("userId")}`}
